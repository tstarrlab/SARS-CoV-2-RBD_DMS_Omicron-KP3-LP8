...
dt_spike[,expression.ysd:=as.numeric(NA)]
for(i in 1:nrow(dt_spike)){
  if(dt_spike[i,site] %in% dt_final[target=="Omicron_KP3",position]){
    ...
    dt_spike[i,"expression.ysd"] <- dt_final[target=="Omicron_KP3" & position==dt_spike[i,site] & mutant==dt_spike[i,mutant],delta_expr]
  }
}
...
r2_labels_expr <- dt_spike %>%
  filter(region %in% c("RBD_distal", "RBD_proximal", "RBD_contact")) %>%
  group_by(region) %>%
  summarise(
    r2 = summary(lm(ACE2.binding ~ expression.ysd))$r.squared,
    y = max(dt_spike[region %in% c("RBD_distal", "RBD_proximal", "RBD_contact"),ACE2.binding], na.rm = TRUE),  # adjust label position
    x = min(dt_spike[region %in% c("RBD_distal", "RBD_proximal", "RBD_contact"),expression.ysd], na.rm = TRUE)
  )
```
...
ggplot(dt_spike[region %in% c("RBD_distal", "RBD_proximal", "RBD_contact") & !is.na(region)], 
  ...
  labs(
    ...
    x = "RBD DMS effect, ACE2-binding affinity"
  ) +
  theme_classic()
```
...
#mutations to label
dt_labels <- rbind(
  dt_spike[region == "RBD_proximal" & expression.ysd > -0.75 & ACE2.binding > 1, .(ACE2.binding, expression.ysd, mutation, region)],
  dt_spike[region == "RBD_proximal" & expression.ysd > -0.5 & ACE2.binding < -2, .(ACE2.binding, expression.ysd, mutation, region)]
)

dt_labels$region <- factor(dt_labels$region,levels=c("RBD_distal","RBD_proximal","RBD_contact"))

ggplot(dt_spike[region %in% c("RBD_distal", "RBD_proximal", "RBD_contact") & !is.na(region)], 
       aes(y = ACE2.binding, x = expression.ysd)) +
  geom_point(alpha = 0.4, size = 2, shape = 16, color="gray40") +
  geom_smooth(method = "lm", se = FALSE, linetype = "dashed", color = "black") +
  geom_text_repel(
    data = dt_labels,
    aes(x=expression.ysd,y=ACE2.binding,label = mutation),
    size = 2.5,
    force=1.5,
    min.segment.length=0,
    color = "black",
  ...
  labs(
    y = "spike PV DMS effect, ACE2 neutralization",
    x = "RBD DMS effect, RBD expression"
  ) +
  theme_classic()
```
...
<img src="collapse_scores_files/figure-gfm/plot_spike_yeast-expr_v_ACE2-PV-1.png" style="display: block; margin: auto;" />

``` r
invisible(dev.print(pdf, paste(config$final_variant_scores_dir,"/scatterplots_DMS-traits_RBD-expr_v_spike-ACE2-neut.pdf",sep="")))
```
...
Heatmaps zooming in on specific sites across variants. Want to make it
once as the true dKd, and once as dKds re-calibrated relative to the
same (WH1?) reference state

``` r
#recalibrate d-Kds to be relative to the WH1 wildtype amino acid identity
ref <- dt_final[target == "Wuhan-Hu-1", .(wuhan_wt = wildtype[1]), by = position]
setkey(ref, position)

dt_final[, delta_bind_ref := {
  wwt <- ref[.SD, on=.(position), wuhan_wt]   # Wuhan AA per row
  ref_bind <- dt_final[.(target, position, wwt), on=.(target, position, mutant), bind]
  bind - ref_bind
}]

#make temp long-form data frame
temp_sites <- data.table::melt(dt_final[position %in% c(455,456,493,452) & mutant != "-", .(target,position,mutant,delta_bind,delta_bind_ref,wildtype_indicator)],id.vars=c("target","position","mutant","wildtype_indicator"),measure.vars=c("delta_bind","delta_bind_ref"),variable.name="measurement",value.name="value")

p1 <- ggplot(temp_sites[measurement=="delta_bind_ref",],aes(target,mutant))+geom_tile(aes(fill=value),color="black",lwd=0.1)+
  scale_fill_gradientn(colours=c("#A94E35","#A94E35","#F48365","#FFFFFF","#7378B9","#383C6C","#383C6C"),limits=c(-5,4),values=c(0/9, 1/9, 3/9, 5/9, 6/9, 7/9, 9/9),na.value="yellow")+
  labs(x="",y="")+theme_classic(base_size=9)+
  coord_equal()+theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.6,face="bold",size=10),axis.text.y=element_text(face="bold",size=10))+
...
<img src="collapse_scores_files/figure-gfm/heatmap_DMS_delta-log10Ka-by-target_subset-sites-1.png" style="display: block; margin: auto;" />

``` r
invisible(dev.print(pdf, paste(config$final_variant_scores_dir,"/heatmap_SSM_delta-log10Ka-by-site_455-456-493.pdf",sep="")))
```
