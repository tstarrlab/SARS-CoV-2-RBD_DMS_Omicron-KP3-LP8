...
#temp wh1 replot for grant figure
sites <- read.csv(file=config$RBD_sites,stringsAsFactors = F)
...
for(i in 1:nrow(sites)){
  if(!is.na(sites[i,"site"])){
    sites$mean_bind[i] <- mean(dt_final[position==sites[i,"site"] & target=="Wuhan-Hu-1" & wildtype != mutant & mutant!= "-" & n_bc_bind>1,delta_bind],na.rm=T)
    sites$max_bind[i] <- max(dt_final[position==sites[i,"site"] & target=="Wuhan-Hu-1" & wildtype != mutant & mutant!= "-" & n_bc_bind>1,delta_bind],na.rm=T)
    sites$del_bind[i] <- dt_final[position==sites[i,"site"] & target=="Wuhan-Hu-1" & mutant== "-",delta_bind]
    
    sites$mean_expr[i] <- mean(dt_final[position==sites[i,"site"] & target=="Wuhan-Hu-1" & wildtype != mutant & mutant!= "-" & n_bc_expr>1,delta_expr],na.rm=T)
    sites$max_expr[i] <- max(dt_final[position==sites[i,"site"] & target=="Wuhan-Hu-1" & wildtype != mutant & mutant!= "-" & n_bc_expr>1,delta_expr],na.rm=T)
    sites$del_expr[i] <- dt_final[position==sites[i,"site"] & target=="Wuhan-Hu-1" & mutant== "-",delta_expr]
  }
}

pdb <- read.pdb(file=config$pdb_6m0j)
...
for(i in 1:nrow(pdb$atom)){
  res <- pdb$atom$resno[i]
  if(pdb$atom$chain[i] %in% c("E") & res %in% sites$site){
    b_mean_bind[i] <- sites[sites$site==res,"mean_bind"]
    b_max_bind[i] <- sites[sites$site==res,"max_bind"]
    b_del_bind[i] <- sites[sites$site==res,"del_bind"]
    b_mean_expr[i] <- sites[sites$site==res,"mean_expr"]
    b_max_expr[i] <- sites[sites$site==res,"max_expr"]
    b_del_expr[i] <- sites[sites$site==res,"del_expr"]
  }
}

write.pdb(pdb=pdb, file=paste(config$final_variant_scores_dir,"/pdbs/bind_mean_WH1-temp.pdb",sep=""), b=b_mean_bind)
write.pdb(pdb=pdb, file=paste(config$final_variant_scores_dir,"/pdbs/bind_max_WH1-temp.pdb",sep=""), b=b_max_bind)
write.pdb(pdb=pdb, file=paste(config$final_variant_scores_dir,"/pdbs/bind_del_WH1-temp.pdb",sep=""), b=b_del_bind)
write.pdb(pdb=pdb, file=paste(config$final_variant_scores_dir,"/pdbs/expr_mean_WH1-temp.pdb",sep=""), b=b_mean_expr)
write.pdb(pdb=pdb, file=paste(config$final_variant_scores_dir,"/pdbs/expr_max_WH1-temp.pdb",sep=""), b=b_max_expr)
write.pdb(pdb=pdb, file=paste(config$final_variant_scores_dir,"/pdbs/expr_del_WH1-temp.pdb",sep=""), b=b_del_expr)
...
```{r plot_expr-v-bind, echo=T, fig.width=8, fig.height=4.5, fig.align="center", dpi=300,dev="png"}
par(mfrow=c(1,2))
x <- dt_final[target=="Omicron_BA286",delta_expr]; y <- dt_final[target=="Omicron_BA286",delta_bind]; plot(x,y,pch=16,col="#00000020",xlab="mutation effect on RBD expression (delta-logMFI)",ylab="mutation effect on ACE2 binding (delta-log10Kd)",main="", xlim=c(-2,1),ylim=c(-5,1))
