...
```{r plot_correlations_separate-backgrounds, echo=T, fig.width=8, fig.height=4, fig.align="center", dpi=300,dev="png"}
par(mfrow=c(1,2))
...
```{r plot_expr_correlations_separate-backgrounds, echo=T, fig.width=8, fig.height=4, fig.align="center", dpi=300,dev="png"}
par(mfrow=c(1,2))
...
```{r combine_with_spike, fig.width=5.5,fig.height=4,fig.align="center", dpi=500,dev="png",echo=T}
...
```{r plot_spike_sera-escape_v_ACE2-PV, fig.width=9,fig.height=3.5,fig.align="center", dpi=500,dev="png",echo=T}
...
```{r plot_spike_yeast-ACE2_v_ACE2-PV, fig.width=9,fig.height=3.5,fig.align="center", dpi=500,dev="png",echo=T}
...
```{r plot_spike_yeast-expr_v_ACE2-PV, fig.width=9,fig.height=3.5,fig.align="center", dpi=500,dev="png",echo=T}
...
Compute sitewise metric for difference between ACE2 binding in PV and YSD of effects of mutations at each site

To do this, let's infer a nonlinear function that captures nonlinearity between RBD and PV DMS effect based on the RBD_contact residue set, where this relationship is in its "purest" form. Then we take all RBD residues and compute mutational > sitewise deviation from this relationship. We summarize sitewise metrics as the mean residual (in case there's an open/close bias) and the average magnitude of residual (in case there are equal and opposite opening and closing effects of mutations)

We do this only for mutations where ACE2 binding in the RBD YSD platform is > -2 -- this is where the relationship between spike and RBD ACE2 binding is more reliable, plus mutations below this threshold are not likely relevant to SARS2 evolution anyway

```{r compare_full_spike_mean-per-site, fig.width=3,fig.height=3,fig.align="center", dpi=500,dev="png",echo=T}
...
```
Heatmaps zooming in on specific sites across variants. Want to make it once as the true dKd, and once as dKds re-calibrated relative to the same (WH1?) reference state

```{r heatmap_DMS_delta-log10Ka-by-target_subset-hotspot-sites, fig.width=4,fig.height=10,fig.align="center", dpi=500,dev="png",echo=T}
...
#make temp long-form data frame
temp_sites <- data.table::melt(dt_final[position %in% c(455,456,493) & mutant != "-", .(target,position,mutant,delta_bind,delta_bind_ref,wildtype_indicator)],id.vars=c("target","position","mutant","wildtype_indicator"),measure.vars=c("delta_bind","delta_bind_ref"),variable.name="measurement",value.name="value")
...
```{r heatmap_DMS_delta-log10Ka-by-target_subset-ctrl-sites, fig.width=4,fig.height=10,fig.align="center", dpi=500,dev="png",echo=T}
#recalibrate d-Kds to be relative to the WH1 wildtype amino acid identity
ref <- dt_final[target == "Wuhan-Hu-1", .(wuhan_wt = wildtype[1]), by = position]
setkey(ref, position)
...
#make temp long-form data frame
temp_sites <- data.table::melt(dt_final[position %in% c(452, 498, 501) & mutant != "-", .(target,position,mutant,delta_bind,delta_bind_ref,wildtype_indicator)],id.vars=c("target","position","mutant","wildtype_indicator"),measure.vars=c("delta_bind","delta_bind_ref"),variable.name="measurement",value.name="value")
...
p1
invisible(dev.print(pdf, paste(config$final_variant_scores_dir,"/heatmap_SSM_delta-log10Ka-by-site_452-498-501.pdf",sep="")))
```

Plot mutation effects on expression versus binding in a single scatterplot.

```{r plot_expr-v-bind, echo=T, fig.width=8, fig.height=8, fig.align="center", dpi=300,dev="png"}
par(mfrow=c(2,2))
