...
rule make_summary:
    ...
    input:
        ...
        process_ccs_KP3=nb_markdown('process_ccs_KP3.ipynb'),
        process_ccs_LP8=nb_markdown('process_ccs_LP8.ipynb'),
        barcode_variant_table_KP3=config['codon_variant_table_file_KP3'],
        barcode_variant_table_LP8=config['codon_variant_table_file_LP8'],
    ...
    run:
        ...
        with open(output.summary, 'w') as f:
            ...
            [Omicron LP8]({path(input.process_ccs_LP8)}). Creates barcode-variant lookup tables for each background: [KP3]({path(input.barcode_variant_table_KP3)}), and [LP8]({path(input.barcode_variant_table_LP8)}).
...
rule fit_titrations:
    input:
        config['codon_variant_table_file_KP3'],
        config['codon_variant_table_file_LP8'],
...
rule process_ccs_LP8:
    """Process the PacBio CCSs for LP8 background and build variant table."""
    input:
        config['pacbio_runs']
    output:
        config['processed_ccs_file_LP8'],
    	config['codon_variant_table_file_LP8'],
        nb_markdown=nb_markdown('process_ccs_LP8.ipynb')
    params:
        nb='process_ccs_LP8.ipynb'
    shell:
        "python scripts/run_nb.py {params.nb} {output.nb_markdown}"
        
rule process_ccs_KP3:
    """Process the PacBio CCSs for KP3 background and build variant table."""
    input:
        config['pacbio_runs']
    output:
        config['processed_ccs_file_KP3'],
    	config['codon_variant_table_file_KP3'],
        nb_markdown=nb_markdown('process_ccs_KP3.ipynb')
    params:
        nb='process_ccs_KP3.ipynb'
    shell:
        "python scripts/run_nb.py {params.nb} {output.nb_markdown}"
